...
from model.modeling_chatglm import ChatGLMForSequenceClassification
from model.tokenization_chatglm import ChatGLMTokenizer
from model.configuration_chatglm import ChatGLMConfig
from preprocess import ClassifierDatasetForLLM
...
def run_classifier(
    ...
    config.num_labels = 2
    model = ChatGLMForSequenceClassification.from_pretrained(model_args.model_name_or_path, config)
    # new_tokens = ["[OPS]", "[OPE]"]
    ...
    # train_data = ClassifierDatasetForLLM(os.path.join(data_args.dataset_dir, "classifier_data_math23k.json"),
    #                                  'train', data_args, tokenizer)
    # valid_data = ClassifierDatasetForLLM(os.path.join(data_args.dataset_dir, "classifier_valid_data_math23k.json"),
    #                                  'valid', data_args, tokenizer)
    train_data = ClassifierDatasetForLLM(os.path.join(data_args.dataset_dir, "classifier_data.json"),
                                                'train', data_args, tokenizer)
    valid_data = ClassifierDatasetForLLM(os.path.join(data_args.dataset_dir, "classifier_valid_data.json"),
                                                'valid', data_args, tokenizer)
    ...
    if training_args.do_predict:
        test_data = ClassifierDatasetForLLM(os.path.join(data_args.dataset_dir, "classifier_test_data.json"),
                                             'valid', data_args, tokenizer)
